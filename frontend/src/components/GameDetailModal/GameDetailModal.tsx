...
function GameDetailModal({ game, onClose }: IGameDetailModal) {
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className='w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl p-0'
        showCloseButton
      >
        <div className='flex flex-col sm:flex-row gap-0'>
          ...
          <div className='flex-1 flex flex-col p-5 gap-4 min-w-0'>
            <DialogHeader>
              ...
              <DialogTitle className='text-xl leading-snug mt-1'>
                {shortName}
              </DialogTitle>
            </DialogHeader>
  ...
  function renderMeta() {
    const hasAnyMeta =
      game.releaseDate || genreList.length > 0 || platformList.length > 0;
